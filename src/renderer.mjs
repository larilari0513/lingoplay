...
let config, boot, calls = 0, toastTimer, hasKey = false, overlayShown = false;
let sourceId = '', sourceTitle = '', previewImage, region = { x: 0, y: 0, w: 1, h: 1 }, dragStart = null;
let watching = false, screenBusy = false, screenTimer, screenEpoch = 0, lastSignature, screenText = '', chatResult = '', chatEpoch = 0, chatBusy = false;
...
function count() { calls++; $('#sessionStats').textContent = `이번 실행 · 번역 ${calls}회`; }
...
function stopScreen() { watching = false; screenEpoch++; clearTimeout(screenTimer); $('#screenStart').textContent = '▶ 자동 번역'; $('#screenStart').classList.remove('active'); setScreenStatus('대기 중'); }
...
async function translateScreen(force = false) {
  ...
  try {
    ...
    if (!force && !changed(lastSignature, sig)) { setScreenStatus('화면 변화 기다리는 중', watching); return; }
    ...
    lastSignature = sig; renderBlocks(result.blocks); screenText = result.blocks.map(x => x.translated).join('\n\n'); $('#copyScreen').disabled = !screenText;
    $('#screenTiming').textContent = result.cached ? '이전 번역 재사용' : `${((performance.now() - start) / 1000).toFixed(1)}초 · ${result.blocks.length}개 문단`; if (!result.cached) count();
...
function renderLane(lane, state = 'idle') {
  ...
  $(`#${lane}Status`).textContent = { idle: incoming ? '수신을 시작하면 번역 자막이 표시돼요.' : '한 번 눌러 말하고, 다시 눌러 번역하세요. (최대 20초)', connecting: '오디오와 번역 API를 연결하고 있어요.', streaming: incoming ? '듣는 중 · 실시간으로 번역하고 있어요.' : '말소리를 받고 있어요. 끝나면 말하기 끝을 누르세요.', draining: '녹음은 끝났어요. 인식·번역을 기다려 주세요.' }[state];
  ['micDevice','incomingDevice','outputDevice','enableSend','monitorOriginal','deviceRefresh'].forEach(id => $(`#${id}`).disabled = Boolean(lanes.incoming || lanes.outgoing));
}
...
async function endLane(lane, immediate = false) {
  ...
  session.stopping = true; await session.capture?.stop(!immediate); session.state = 'draining';
  try { await call('voice-stop', { lane, token: session.token, immediate }); } catch (e) { problem(e); }
  renderLane(lane, immediate ? 'idle' : 'draining');
}
...
async function startLane(lane) {
  ...
  const s = { token: crypto.randomUUID(), capture: new AudioCapture(), player: null, caption: createTranscript(lane), state: 'connecting', drained: false, stopping: false };
  ...
  try {
    ...
    await call('voice-bind', { lane, token: s.token }); s.state = 'streaming'; renderLane(lane, 'streaming'); count();
...
api.on('voice-event', e => {
  ...
  if (e.type === 'transcript' || e.type === 'translation') {
    const c = s.caption; if (e.type === 'transcript') { c.original = (c.original + e.delta).slice(-4000); c.source.textContent = c.original; } else { c.translated = (c.translated + e.delta).slice(-4000); c.target.textContent = c.translated; if (!c.original) c.source.textContent = '실시간 번역 · 원문이 제공되면 함께 표시됩니다.'; }
...
function stopEverything() { stopScreen(); chatEpoch++; chatBusy = false; $('#chatTranslate').disabled = false; for (const lane of ['incoming', 'outgoing']) { const s = lanes[lane]; if (s) { lanes[lane] = null; clearTimeout(s.closeTimer); s.capture?.stop().catch(() => {}); s.player?.stop(); } renderLane(lane); } toast('화면 번역과 음성 전송을 모두 중지했어요.'); }
async function translateChat() {
  ...
  try { const result = await call('translate-text', { text, target: $('#chatTarget').value, game: config.game, glossary: config.glossary }); if (epoch !== chatEpoch) return;
    chatResult = result.translated; $('#chatOutput').textContent = chatResult; $('#chatOutput').classList.remove('empty'); $('#chatCopy').disabled = false; $('#chatTiming').textContent = result.cached ? '이전 번역 재사용' : `${((performance.now() - start) / 1000).toFixed(1)}초`; if (!result.cached) count(); history.unshift({ original: text, translated: chatResult }); history = history.slice(0, 30); renderHistory();
...
$('#settingsForm').onsubmit = async event => { event.preventDefault(); if (lanes.incoming || lanes.outgoing) return toast('음성 번역을 마친 뒤 언어를 변경해 주세요.', true); try { stopScreen(); config = await call('save-settings', { ...config, target: $('#targetLanguage').value, outgoing: $('#outgoingLanguage').value, game: $('#gameName').value, glossary: $('#glossary').value, interval: Number($('#interval').value), overlaySize: Number($('#overlaySize').value), mic: $('#micDevice').value, incoming: $('#incomingDevice').value, output: $('#outputDevice').value }); applySettings(); lastSignature = null; $('#settingsDialog').close(); toast('설정을 저장했어요.'); } catch (e) { problem(e); } };
$('#saveKey').onclick = async () => { try { if (!$('#apiKey').value.trim()) return toast('저장할 키를 입력해 주세요.'); setKeyState(await call('save-key', $('#apiKey').value)); $('#apiKey').value = ''; $('#apiCheckResult').textContent = ''; toast('API 키를 암호화해 저장했어요.'); } catch (e) { problem(e); } };
$('#removeKey').onclick = async () => { try { setKeyState(await call('save-key', '')); toast('앱에 저장한 API 키를 지웠어요.'); } catch (e) { problem(e); } };
$('#checkApi').onclick = async () => { $('#checkApi').disabled = true; $('#apiCheckResult').textContent = '확인 중…'; try { const result = await call('check-api'); $('#apiCheckResult').textContent = result.text && result.voice ? '연결 확인 · 화면·채팅·음성 모델 사용 가능' : `연결됨 · ${!result.text ? '텍스트 모델 ' : ''}${!result.voice ? '음성 모델 ' : ''}권한 확인 필요`; } catch (e) { $('#apiCheckResult').textContent = '연결 실패'; problem(e); } finally { $('#checkApi').disabled = false; } };
